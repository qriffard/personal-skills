...
## Local knowledge vault integration

The pipeline is wiki-aware. It reads from **all registered LLM Wikis** to
ground the research in prior knowledge and avoid redundant work.

**Vault discovery:**
Read the `## LLM Wikis` registry in `~/.claude/CLAUDE.md` (between the
`<!-- BEGIN/END: create-llm-wiki registry -->` markers). Each entry has a name
and absolute path. Collect all vault paths. If the registry is missing or empty,
fall back to listing `~/wikis/*/` directories that contain an `index.md`.
If no vaults are found, skip vault integration silently.

**Phase 0 — Prior knowledge lookup:**
1. For each discovered vault, read its `index.md` and scan for pages related to
   the topic.
2. Across all vaults, select up to 5 relevant pages total (prioritize by
   relevance, not vault order). Read them from their respective `wiki/` dirs.
3. Compile a `{PRIOR_CONTEXT}` block (key claims, existing findings, known gaps,
   noting which vault each came from) and append it to each lens prompt in
   Phase 1 as:
   `PRIOR KNOWLEDGE (from local research vaults — reference but do not limit yourself to this): {PRIOR_CONTEXT}`

**Phase 4c — Cross-reference citations against vaults:**
After verification, for each vault, check whether any cited source URLs already
exist by grepping `<vault>/wiki/**/*.md` frontmatter `source_url:` fields and
listing filenames in `<vault>/raw/`.

In the final chat output, add a section:
- **Already in vault**: list matched sources with their vault name and wiki page
  slug.
- **New — worth ingesting**: list 1-3 high-value sources not yet captured that
  would compound existing knowledge, noting which vault they'd belong in.
  (Do not auto-ingest; just recommend.)
...
Spawn **five `general-purpose` agents in a single message** so they run concurrently. Each gets the SAME topic framing plus its own lens. Use these exact prompts, substituting `{TOPIC}` and a one-line `{TOPIC_FRAME}` (your Phase 0 interpretation). If Phase 0 produced a `{PRIOR_CONTEXT}` block from the wiki, append it to each lens prompt.
...
3. In chat, give: the file path, the verification tally (`N/N checked, X fabricated, Y corrected, Z demoted`), the one universal finding, the frontier question, the claim safety summary (what is safe to assert vs avoid), and the vault cross-reference (already captured vs new sources worth ingesting). Keep it tight.
